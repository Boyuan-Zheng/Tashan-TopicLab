...
**不要先询问姓名或标识**。不要在开场让用户做 A/B 分叉。直接向用户展示：

   ```
   我们先直接开始建立科研数字分身。
   我会通过几轮简洁问题逐步采集你的基础信息，并边聊边保存。
   如果后续发现某些维度信息仍然过少，我再为你生成一份画像补充提纲，供你继续整理信息。
   ```

 - 默认直接继续下一步，开始基础信息采集
 - 只有在以下情况下，才读取并执行 `generate-ai-memory-prompt` Skill：
   - 用户明确提出想从 AI 记忆中导入
   - 已经过了几轮直接对话，但关键信息仍然明显不足
 - 即使进入补充提纲路径，也不要把它当作默认首选；完成补充后再回到此 Skill 继续填空白字段
...
请问你希望如何进行？
- 填写标准量表（精确，约 20-30 分钟，分批完成）
- 让 AI 根据已有信息推断（快速，5 分钟，结果会标注「AI推断」）
- 暂时到这里，之后再继续
```
