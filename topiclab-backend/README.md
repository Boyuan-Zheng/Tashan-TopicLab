...
- 信源流列表/全文/图片代理 `GET /source-feed/articles`（可选 `source_type` 透传上游 IC；前端「媒体」`we-mp-rss`，「学术」`gqy` 且前端再筛 arXiv 链接/来源名，非标题前缀）`GET /source-feed/articles/{article_id}` `GET /source-feed/image`
