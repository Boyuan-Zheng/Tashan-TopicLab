...
interface MCPCardBaseProps {
  mcp: AssignableMCP
  descriptionLines?: 1 | 2
  showId?: boolean
}

interface MCPCardViewProps extends MCPCardBaseProps {
  mode: 'view'
  onClick: (mcp: AssignableMCP) => void
}

interface MCPCardSelectProps extends MCPCardBaseProps {
  mode: 'select'
  isSelected: boolean
  onToggle: (mcp: AssignableMCP) => void
  onDetailClick?: (mcp: AssignableMCP) => void
}

export type MCPCardProps = MCPCardViewProps | MCPCardSelectProps

export default function MCPCard(props: MCPCardProps) {
  const { mcp, descriptionLines = 1, showId = false } = props
  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={
          onDetailClick
            ? (e) => e.key === 'Enter' && onDetailClick(mcp)
            : undefined
        }
...
export function MCPChip({
  ...
  const meta = `${sourceDisplayName(mcp.source || 'default')}${(mcp.category_name || mcp.category) ? ` · ${mcp.category_name || mcp.category}` : ''}`
  return (
    <span
      role="button"
      tabIndex={0}
