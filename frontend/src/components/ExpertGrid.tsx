      ...
      {sourceOrder.map((source) => {
        ...
        return (
          <div key={source} className="border-b last:border-b-0" style={{ borderColor: 'var(--border-default)' }}>
            ...
            >
              <h3
                className="text-xs font-serif font-semibold uppercase tracking-wide"
...
export default function ExpertGrid(props: ExpertGridProps) {
  ...
  const searchInput = (
    <input
      ...
      onBlur={(e) => {
        e.target.style.borderColor = isFill ? 'var(--border-default)' : 'var(--border-default)'
      }}
    />
  )
  ...
  return (
    <div className={rootClass}>
      ...
      {!loading && filteredExperts.length === 0 && (
        <p className="font-serif text-sm" style={{ color: 'var(--text-tertiary)' }}>
          {search ? '无匹配角色' : '暂无角色配置'}
        </p>
      )}
